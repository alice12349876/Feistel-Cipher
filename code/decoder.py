...
def roundCalculation(stringBit, curRound, keyBit):
    a = xor(ffunction(stringBit[0:32], curRound, keyBit), stringBit[32:]) + stringBit[0:32]
    return a
...
def sbox(a):
    # a is a 48 bit input
    # each S box corresponds to one group of 6-bit input
    ...
    for i in range(1,9):
        ...
        # 4 bit output for each group of 6-bit input
    return newString
...
if (len(key) == 56):
    segments = []
    # Divide cipheredText into subgroups of 64 bits
